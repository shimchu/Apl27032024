# <u>**ASSIGNMENT 1 - Matrix Multiplication**

The goal of this assignment is to implement an efficient matrix mulitplication algorithm in Python with proper error handling and edge case management.

This repository provides a clean, modular approach to multiplying matrices while gracefully managing invalid inputs and dimensional inconsistencies.

## **Repository Contents**

matmul.ipynb – Implements the matrix multiplication logic with built-in checks for edge cases such as incompatible matrix sizes and irregular inputs.

testmatmul.py - Unit tests verifying correctness and performance across different matrix types.
